...
class TableOrders extends J2StoreTable
{
	...
	function calculateShippingTotals()
	{
		...
		{
			$this->order_shipping       = $order_shipping_total;
			return;
		}
		...
		foreach ($items as $key=>$item)
		{
			...
			$product_shipping = $shipping_helper->getItemShipping($item);

			$item->orderitem_shipping = $product_shipping->shipping_rate_price + $product_shipping->shipping_rate_handling;
			// track the running total
			$order_shipping_total += $item->orderitem_shipping;
	...
	function getItems()
	{
		...
		if (empty($this->_items) && !empty($this->id))
		{
			...
			$model = JModel::getInstance( 'OrderItems', 'J2StoreModel' );
