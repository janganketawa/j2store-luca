 ...
 * @since 1.5
 */
class J2StoreModelShippingMethods extends JModel
{
	/**
	 *
	 * @var array
	...
	{
		$db = &JFactory::getDBO();
		//$query = 'SELECT a.* FROM #__j2store_shippingmethods AS a WHERE a.published=1 ORDER BY a.id';
		$query = $this->_buildQuery();
		$db->setQuery($query);
	...
	function _buildQuery()
	{
		...
		$query = ' SELECT a.*'
			. ' FROM #__j2store_shippingmethods AS a '
			. $where
			. $orderby
		;
	...
	function delete($cid = array())
	{
		...
		if (count( $cid ))
		{
			...
			$cids = implode( ',', $cid );
			$query = 'DELETE FROM #__j2store_shippingmethods'
				. ' WHERE id IN ( '.$cids.' )';
	...
	function publish($cid = array(), $publish = 1)
	{
		...
		if (count( $cid ))
		{
			...
			$query = 'UPDATE #__j2store_shippingmethods'
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query = 'SELECT a.* FROM #__j2store_shippingmethods AS a' .
	...
	function getShippingMethodType($type)
	{
		...
		$shipping_types=Array(0=>"J2STORE_SHIPM_FLAT_RATE_PER_ORDER",
					1=>"J2STORE_SHIPM_QUANTITY_BASED_PER_ORDER",
					2=>"J2STORE_SHIPM_PRICE_BASED_PER_ORDER");
