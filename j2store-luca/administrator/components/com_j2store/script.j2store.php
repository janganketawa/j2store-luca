...
//		$jversion = new JVersion();
	...
	function update($parent) {
		$db = & JFactory::getDBO();
		//get the backup done
		require_once (JPATH_ADMINISTRATOR.DS.'components'.DS.'com_j2store'.DS.'library'.DS.'backup.php');
		...
		//orderfiles
		if(!in_array($prefix.'j2store_orderfiles', $tables)){
			$query = "
		...
		//price table modifications - j2store 1.2.1 build
		...
		$oldRelease = $tmpInstaller->get( "manifest" )->version;
		
		 	if($oldRelease == '1.1.0' || $oldRelease == '1.2.0' ) {
				$sql = 'UPDATE #__j2store_prices SET product_enabled=1 WHERE item_price > 0';
				$db->setQuery($sql);
				$db->query();
		 	}			
		...
		//address table modifications
		if(in_array($prefix.'j2store_orderfiles', $tables)){
			$address_fields = $db->getTableColumns('#__j2store_orderfiles');

			if (!array_key_exists('user_email', $address_fields)) {
				$query = "ALTER TABLE #__j2store_orderfiles ADD `user_email` varchar(255) NOT NULL AFTER `user_id`";
				$db->setQuery($query);
				$db->query();
			}
		}
		...
		//country table
		//if(!in_array($prefix.'j2store_countries', $tables)){
			$query = "
	...
	//	if(!in_array($prefix.'j2store_zones', $tables)){
